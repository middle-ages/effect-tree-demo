import {flow, pipe, type EndoOf, type LazyArg} from '#Function'
import type {Predicate} from 'effect/Predicate'
import type {Pair} from '#Pair'
import {pluck} from '#Record'
import * as Boolean from 'effect/Boolean'

const [poolMin, poolMax] = [5, 8]
...
export const abortAll = () => {
  for (const entry of filterBusy()) {
    abortWorker(entry)
  }
}

export const withWorker = <Result>(
...
const filter = (predicate: Predicate<WorkerEntry>): WorkerEntry[] => [
  ...workerMap
    .entries()
...
const [filterBusy, filterAvailable]: Pair<LazyArg<WorkerEntry[]>> = [
  () => filter(pluck('isBusy')),
  () => filter(flow(pluck('isBusy'), Boolean.not)),
]

export const [countBusy, countAvailable]: Pair<LazyArg<number>> = [
  () => filterBusy().length,
...
export const poolReport = () => ({
  busy: countBusy(),
  available: countAvailable(),
})

export const bootPool = () => {
